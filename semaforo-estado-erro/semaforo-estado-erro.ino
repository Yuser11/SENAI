...
void normal()
{
  ...
  }  if ((millis() - millisTarefa1) < amarelo1 && (millis() - millisTarefa1) > verde1) {
    digitalWrite(amrl1, HIGH);
    digitalWrite(vd1, LOW);
  }  if ((millis() - millisTarefa1) < vermelho1 && (millis() - millisTarefa1) > amarelo1) {
  ...
  }  if ((millis() - millisTarefa2) < verde2 && (millis() - millisTarefa1) > vermelho2) {
    digitalWrite(vm2, LOW);
    digitalWrite(vd2, HIGH);
  }  if ((millis() - millisTarefa2) < amarelo2 && (millis() - millisTarefa1) > verde2) {
    digitalWrite(amrl2, HIGH);
    digitalWrite(vd2, LOW);
  }  if ((millis() - millisTarefa1) > amarelo2 && (millis() - millisTarefa1) > vermelho1) {
...
void oscilando()
{
  if ((millis() - millisTarefa1) < (amarelo1 - verde1)) {
    digitalWrite(amrl1, HIGH);
    digitalWrite(amrl2, HIGH);
  } else if ((millis() - millisTarefa1) < (amarelo1 - verde1 + 1000) && (millis() - millisTarefa1) > (amarelo1 - verde1)) {
    digitalWrite(amrl1, LOW);
    digitalWrite(amrl2, LOW);
  } else  {
    millisTarefa1 = millis();
...
void sistema() {
  ...
  {  
    ...
    {
      if (resposta == false) {
        Serial.println("Estados\n1-)Normal, 2-)Oscilando, 3-)Semprecor1, 4-)Semprecor2");
        resposta = true;
      }  else {
        tempEstado = Serial.parseInt();
        Serial.print(estado);
